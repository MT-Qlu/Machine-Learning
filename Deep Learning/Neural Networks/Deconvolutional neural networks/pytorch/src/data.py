"""Data loading utilities for the deconvolutional autoencoder."""
from __future__ import annotations

from typing import Tuple

import torch
from torch.utils.data import DataLoader
from torchvision import datasets, transforms

from .config import CONFIG, DeconvConfig


def _transforms() -> transforms.Compose:
    return transforms.Compose(
        [
            transforms.ToTensor(),
            transforms.Normalize((0.5,), (0.5,)),
        ]
    )


def load_datasets(config: DeconvConfig = CONFIG) -> Tuple[datasets.FashionMNIST, datasets.FashionMNIST]:
    transform = _transforms()
    train_ds = datasets.FashionMNIST(
        root=str(config.data_dir),
        train=True,
        download=True,
        transform=transform,
    )
    val_ds = datasets.FashionMNIST(
        root=str(config.data_dir),
        train=False,
        download=True,
        transform=transform,
    )
    return train_ds, val_ds


def create_dataloaders(
    config: DeconvConfig = CONFIG,
) -> Tuple[DataLoader[torch.Tensor], DataLoader[torch.Tensor]]:
    train_ds, val_ds = load_datasets(config)
    train_loader = DataLoader(
        train_ds,
        batch_size=config.batch_size,
        shuffle=True,
        num_workers=config.num_workers,
        pin_memory=config.device.type != "cpu",
    )
    val_loader = DataLoader(
        val_ds,
        batch_size=config.batch_size,
        shuffle=False,
        num_workers=config.num_workers,
        pin_memory=config.device.type != "cpu",
    )
    return train_loader, val_loader
