...
**Location:** `Machine-Learning/Essentials/metrics`

This guide consolidates the most commonly used loss and error metrics across regression, classification, and forecasting workflows. Each entry includes the mathematical definition, a plain-text counterpart for quick reference, typical use cases, and implementation notes that align with the modular utilities in `Essentials/metrics/metrics.py`.

## Getting Started

```python
from Essentials.metrics.metrics import mean_absolute_error, root_mean_squared_error
...
Refer to `Essentials/metrics/metrics.py` for ready-to-use implementations and integration tips.
