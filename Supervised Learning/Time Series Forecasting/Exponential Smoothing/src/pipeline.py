"""Training pipeline utilities for the Exponential Smoothing forecaster."""
...
class AirPassengersExponentialSmoothingPipeline:
    """Compose Holt-Winters training, evaluation, and persistence."""
    ...
    def train(self) -> dict[str, float]:
        ...
        results = model.fit()
        forecast = results.forecast(len(test_series))
    ...
    def forecast(self, horizon: int) -> pd.Series:
        ...
        forecast_values = self.model.forecast(horizon)
    ...
    @staticmethod
    def load(path: Path | None = None) -> "AirPassengersExponentialSmoothingPipeline":
        pipeline_path = path or CONFIG.model_path
        payload = joblib.load(pipeline_path)
        pipeline = AirPassengersExponentialSmoothingPipeline(payload["config"])
...
def train_and_persist(config: ExponentialSmoothingConfig | None = None) -> dict[str, float]:
    pipeline = AirPassengersExponentialSmoothingPipeline(config)
