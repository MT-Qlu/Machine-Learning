"""Inference utilities for the AirPassengers SARIMA forecaster."""
...
from .pipeline import AirPassengersSARIMAPipeline, train_and_persist


class SARIMAForecastRequest(BaseModel):
    """Request payload for SARIMA forecasts."""
...
class SARIMAForecastService:
    """High-level service that wraps SARIMA inference."""
    ...
    def _load_or_train(self) -> AirPassengersSARIMAPipeline:
        if not self.config.model_path.exists():
            train_and_persist(self.config)
        return AirPassengersSARIMAPipeline.load(self.config.model_path)
    ...
    def predict(self, payload: SARIMAForecastRequest) -> SARIMAForecastResponse:
...
@lru_cache(maxsize=1)
def get_service() -> SARIMAForecastService:
    return SARIMAForecastService()


RequestModel = SARIMAForecastRequest
ResponseModel = SARIMAForecastResponse
