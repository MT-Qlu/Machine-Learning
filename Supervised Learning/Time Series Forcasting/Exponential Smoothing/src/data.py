"""Data loading utilities for the Exponential Smoothing module."""
...
def load_dataframe(config: ExponentialSmoothingConfig = CONFIG) -> pd.DataFrame:
...
def load_series(config: ExponentialSmoothingConfig = CONFIG) -> pd.Series:
    df = load_dataframe(config)
    return df[config.target_column]


def train_test_split(config: ExponentialSmoothingConfig = CONFIG) -> Tuple[pd.Series, pd.Series]:
