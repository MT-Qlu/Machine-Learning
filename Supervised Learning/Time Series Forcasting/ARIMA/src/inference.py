"""Inference utilities for the AirPassengers ARIMA forecaster."""
...
from .pipeline import AirPassengersARIMAPipeline, train_and_persist


class ARIMAForecastRequest(BaseModel):
    """Request payload for time-series forecasts."""

    horizon: int = Field(12, ge=1, le=60, description="Number of future periods to forecast")
...
class ARIMAForecastService:
    """High-level service that wraps ARIMA model inference."""
    ...
    def _load_or_train(self) -> AirPassengersARIMAPipeline:
        if not self.config.model_path.exists():
            train_and_persist(self.config)
        return AirPassengersARIMAPipeline.load(self.config.model_path)
    ...
    def predict(self, payload: ARIMAForecastRequest) -> ARIMAForecastResponse:
        ...
        return ARIMAForecastResponse(
            ...
            index=[timestamp.strftime("%Y-%m-%d") for timestamp in forecast_series.index],
...
@lru_cache(maxsize=1)
def get_service() -> ARIMAForecastService:
    return ARIMAForecastService()


RequestModel = ARIMAForecastRequest
ResponseModel = ARIMAForecastResponse
